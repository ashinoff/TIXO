...
export async function DELETE(_request: Request, context: Context) {
  ...
  try {
    await ensureSchema();
    const result = await getPool().query("UPDATE products SET archived=TRUE,published=FALSE,updated_at=NOW() WHERE id=$1 AND NOT archived", [integer((await context.params).id, "Товар", 1)]);
