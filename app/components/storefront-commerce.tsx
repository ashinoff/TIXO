  ...
  for (const raw of value.slice(0, 100)) {
    ...
    if (validRecipe(raw.customRecipe)) {
      ...
      lines.set(key, { key, customRecipe: recipe, quantity: raw.quantity,
        ...
        twoTone: raw.twoTone === true, accentColor: typeof raw.accentColor === "string" ? raw.accentColor : undefined, accentColorName: typeof raw.accentColorName === "string" ? raw.accentColorName : undefined, colorName: typeof raw.colorName === "string" ? raw.colorName : undefined,
        color: recipe.colorId ? (typeof raw.color === "string" ? raw.color : undefined) : atelierColorHex[recipe.color] });
...
function visualData(product: Product, color?: CandleColor): CandleVisualData {
  return { twoTone: product.form?.twoTone, accentColor: product.accentColor?.hex, accentColorName: product.accentColor?.name, silhouette: product.form?.silhouette, shape: product.form ? product.form.shape : productShape(product), color: color?.hex ?? product.color?.hex };
}
function CandleVisual({ src, label, silhouette, shape, color, twoTone, accentColor }: CandleVisualData & { src?: string | null; label: string }) {
  return src ? <img src={src} alt={label} width="1254" height="1254" loading="lazy" /> : <div className="candle-silhouette-visual"><CandlePreview silhouette={silhouette} shape={shape} color={color} twoTone={twoTone} accentColor={accentColor} label={`${label} · силуэт формы`} /></div>;
}
...
export function ShoppingProvider({ children }: { children: ReactNode }) {
  ...
  const items = useMemo<DisplayLine[]>(() => cart.map(line => {
    if (isCustom(line)) {
      ...
      const color = line.customRecipe.colorId ? colors.find(color => color.id === line.customRecipe.colorId) : undefined;
      const accent = line.customRecipe.accentColorId ? colors.find(color => color.id === line.customRecipe.accentColorId) : undefined;
      const paletteReady = (line.customRecipe.colorId === undefined || !!color?.active) && (line.customRecipe.accentColorId === undefined || !!accent?.active) && Boolean(form?.twoTone) === Boolean(line.customRecipe.accentColorId);
      const colorName = color?.name ?? line.colorName ?? atelierColors[line.customRecipe.color];
      const accentColorName = accent?.name ?? line.accentColorName;
      return { key: line.key, name: "Моя свеча ТИХО", twoTone: form?.twoTone ?? line.twoTone, accentColor: accent?.hex ?? line.accentColor, accentColorName, colorName, formName, silhouette: form ? form.silhouette : line.silhouette, shape: form ? form.shape : line.shape,
        color: color?.hex ?? line.color ?? atelierColorHex[line.customRecipe.color], scentName: recipeSummary(line.customRecipe, formName, scent?.name ?? line.scentName, colorName, accentColorName), quantity: line.quantity, image: null, price: null,
        available: paletteReady && (line.customRecipe.formId === undefined || (!formsLoading && !formsError && form?.active)) && (line.customRecipe.scentId === undefined || (!loading && !catalogError && scent?.active)) ? 99 : 0, customRecipe: line.customRecipe };
    }
    ...
    const compatible = product && scent && color && matchesCandle(product, scent.id, color.id) && (product.accentColorId ?? null) === (line.accentColorId ?? null) && (product.hasVariants ? variant?.active && variant.scent.active && variant.color.active && variant.scentId === scent.id && variant.colorId === color.id : line.variantId === null);
    ...
    return { ...(product ? visualData(product, color) : { silhouette: line.silhouette, shape: line.shape, color: line.color, twoTone: line.twoTone, accentColor: line.accentColor }), key: line.key, name: product?.name ?? line.name, scentName: `${scent?.name ?? line.scentName} · ${color?.name ?? line.colorName}${(product?.accentColor?.name ?? line.accentColorName) ? ` / ${product?.accentColor?.name ?? line.accentColorName}` : ""}`, quantity: line.quantity, image: product ? photograph(product, color, scent).src : line.image, price: product?.price ?? line.price, available: Math.max(0, stock - otherQuantity) };
  ...
  const addProduct = (product: Product, scent: Scent, color: CandleColor, quantity = 1) => {
    ...
    const variant = variantFor(live, active, color); const key = cartKey(live.id, variant?.id ?? null, active.id, color.id, live.accentColorId);
    ...
    const line: StandardLine = { ...visualData(live, color), accentColorId: live.accentColorId, key, productId: live.id, variantId: variant?.id ?? null, scentId: active.id, colorId: color.id, colorName: color.name, quantity: nextQuantity, name: live.name, scentName: active.name, image: photograph(live, color, active).src, price: live.price };
  ...
  const addCustom = (recipe: Recipe) => {
    ...
    const color = recipe.colorId ? colors.find(color => color.id === recipe.colorId) : undefined;
    const accent = recipe.accentColorId ? colors.find(color => color.id === recipe.accentColorId) : undefined;
    if ((recipe.colorId && !color?.active) || (recipe.accentColorId && !accent?.active) || Boolean(form?.twoTone) !== Boolean(recipe.accentColorId)) { notify("Выберите доступные цвета для этой формы."); return false; }
    ...
    setCart([...cartRef.current.filter(line => line.key !== key), { key, customRecipe: snapshot, quantity, scentName: scent?.name, formName: form?.name, silhouette: form?.silhouette, shape: form?.shape, twoTone: form?.twoTone, color: color?.hex ?? atelierColorHex[recipe.color], colorName: color?.name ?? atelierColors[recipe.color], accentColor: accent?.hex, accentColorName: accent?.name }]); setReceipt(null); setOrderError(""); notify("Ваша авторская свеча — в корзине."); return true;
  };
...
function ScentChapters({ scent, note, onSelect }: { scent: Scent; note: number; onSelect: (note: number) => void }) {
  ...
  return <div className="scent-chapters">
    ...
    <div className="note-panel" key={note} id="note-panel" role="tabpanel" aria-labelledby={`note-tab-${note}`} tabIndex={0}>
      ...
      {stage.description && <p className="note-mobile-description">{stage.description}</p>}
    </div>
  </div>;
}
...
function ProductDetailContent({ product }: { product: Product }) {
  ...
  const carousel = useRef<PhotoCarouselHandle>(null);
  const swipeRef = useProductSwipe({ onPhoto: offset => carousel.current?.move(offset), onPhotoDrag: distance => carousel.current?.drag(distance), onPhotoCancel: () => carousel.current?.cancel(), onProduct: shop.moveProduct, onClose: shop.closeProduct });
  return <div ref={swipeRef} className="product-detail-body" data-product-id={product.id}>{photos.length ? <PhotoCarousel photos={photos} label={`${product.name}${color ? ` · ${color.name}` : ""}`} controlsRef={carousel} /> : <div className="detail-gallery" role="region" aria-label="Фотографии свечи"><div className="detail-photo"><CandleVisual {...visualData(product, color)} label={product.name} /></div></div>}<div className="detail-content"><div className="detail-kicker"><span className="eyebrow">СВЕЧА РУЧНОЙ РАБОТЫ</span><span className="eyebrow">ТИХО</span></div><h2>{product.name}</h2><p>{product.notes}</p>
    ...
    <dl className="detail-specs"><div><dt>Аромат</dt><dd>{scent?.name ?? "Выберите аромат"}</dd></div><div><dt>Ноты</dt><dd>{scent?.notes.join(" · ") || "Выберите аромат"}</dd></div><div><dt>{product.accentColor ? "Корпус" : "Цвет"}</dt><dd>{color?.name ?? "Выберите цвет"}</dd></div>{product.accentColor && <div><dt>Декор</dt><dd>{product.accentColor.name}</dd></div>}<div><dt>Форма</dt><dd>{product.form?.name ?? candleShapes[productShape(product)]}</dd></div><div><dt>Доступно</dt><dd>{!scent || !color ? "Выберите цвет и аромат" : available ? `${available} шт.` : "Это сочетание пока недоступно"}</dd></div></dl><div className="detail-footer"><span>{money(product.price)}</span><div className="qty-picker" aria-label="Количество свечей"><button type="button" aria-label="Уменьшить количество" disabled={quantity <= 1} onClick={() => setQuantity(value => value - 1)}>−</button><output aria-live="polite">{quantity}</output><button type="button" aria-label="Увеличить количество" disabled={quantity >= Math.min(99, available)} onClick={() => setQuantity(value => value + 1)}>+</button></div></div><button type="button" className="button button-dark" disabled={!scent || !color || quantity > available || shop.locked} onClick={() => { if (scent && color && shop.addProduct(product, scent, color, quantity)) shop.closeProduct(); }}>Добавить в корзину <span aria-hidden="true">+</span></button>{photo.preview && <p className="detail-notice">Показан силуэт формы в выбранном цвете. Фотографию готовой свечи можно уточнить у мастерской.</p>}<p className="detail-notice">После оформления мастерская подтвердит заказ и согласует доставку.</p></div></div>;
}
...
function CustomPreview({ recipe, silhouette, shape, formName, color, colorName, twoTone, accentColor }: CandleVisualData & { recipe: Recipe; formName?: string }) {
  if (recipe.formId !== undefined) return <div className="cart-custom-preview"><CandlePreview silhouette={silhouette} shape={shape} color={color ?? atelierColorHex[recipe.color]} twoTone={twoTone} accentColor={accentColor} label={`${recipeFormName(recipe, formName)}, ${colorName ?? atelierColors[recipe.color]}`} /></div>;
