...
import { availableVariants, candleShapes, cartKey, money, productShape, type Product, type Scent, type Variant } from "@/lib/catalog";
...
type StandardLine = { key: string; productId: number; variantId: number | null; scentId: number; quantity: number; name: string; scentName: string; image: string | null; price: number };
type CustomLine = { key: string; customRecipe: Recipe; quantity: number };
type Line = StandardLine | CustomLine;
type DisplayLine = { key: string; name: string; scentName: string; quantity: number; image: string | null; price: number | null; available: number; customRecipe?: Recipe };
type OrderPayload = { requestKey: string; customerName: string; phone: string; email: string; address: string; delivery: string; comment: string; items: ({ productId: number; variantId: number | null; scentId: number; quantity: number } | { customRecipe: Recipe; quantity: number })[] };
type Receipt = { orderNumber: string; total: number; quotePending: boolean };
type Selection = { productId: number; scentId?: number };
type Shop = {
  products: Product[]; scents: Scent[]; loading: boolean; catalogError: string; loadCatalog: () => Promise<void>;
  activeScent: number | null; setActiveScent: (id: number) => void; selectedScent?: Scent;
  cart: Line[]; items: DisplayLine[]; count: number; total: number; hasCustom: boolean; locked: boolean;
  addProduct: (product: Product, scent: Scent, quantity?: number) => boolean; addCustom: (recipe: Recipe) => boolean;
  changeQuantity: (key: string, delta: number) => void; remove: (key: string) => void;
  cartOpen: boolean; setCartOpen: (open: boolean) => void; detail: Selection | null; openProduct: (productId: number, scentId?: number) => void; closeProduct: () => void;
...
function restoreCart(value: unknown): Line[] {
  ...
  for (const raw of value.slice(0, 100)) {
    ...
    if (validRecipe(raw.customRecipe)) {
      ...
      const key = cartKey(raw.productId, raw.variantId, raw.scentId);
      lines.set(key, { key, productId: raw.productId, variantId: raw.variantId, scentId: raw.scentId, quantity: raw.quantity, name: raw.name.slice(0, 160), scentName: raw.scentName.slice(0, 120), image: typeof raw.image === "string" ? raw.image : null, price: raw.price });
    }
  }
  return [...lines.values()];
}
...
function variantFor(product: Product, scent: Scent): Variant | undefined { return availableVariants(product).find(variant => variant.scentId === scent.id); }
function stockFor(product: Product, scent: Scent) { return product.hasVariants ? variantFor(product, scent)?.stock ?? 0 : product.stock; }
function profileFor(product: Product, scents: Scent[], preferred?: number | null) {
  const compatible = product.hasVariants ? availableVariants(product).map(variant => variant.scent) : scents;
  return compatible.find(scent => scent.id === preferred) ?? compatible.find(scent => stockFor(product, scent) > 0) ?? compatible[0];
}
function photograph(product: Product, scent?: Scent) {
  const uploaded = (scent && variantFor(product, scent)?.image) || product.image;
  if (uploaded) return { src: uploaded, preview: false };
  const color = scent?.color.toLowerCase() ?? "";
  const colorName = scent?.colorName.toLowerCase() ?? "";
  const src = /ч[её]рн|уголь|графит/.test(colorName) || /^#([0123][\da-f]){3}$/.test(color) ? "/assets/hero.png" : /красн|винн|бордо/.test(colorName) ? "/assets/red.png" : /роз/.test(colorName) ? "/assets/pink.png" : "/assets/white.png";
  return { src, preview: true };
}
...
export function ShoppingProvider({ children }: { children: ReactNode }) {
  const [products, setProducts] = useState<Product[]>([]); const [scents, setScents] = useState<Scent[]>([]);
  ...
  const loadCatalog = useCallback(async () => {
    try {
      ...
      setProducts(items.filter((product: Product) => product.published)); setScents(profiles.filter((scent: Scent) => scent.active)); setCatalogError("");
  ...
  const items = useMemo<DisplayLine[]>(() => cart.map(line => {
    ...
    const product = products.find(product => product.id === line.productId); const scent = scents.find(scent => scent.id === line.scentId);
    const variant = product?.variants.find(variant => variant.id === line.variantId);
    const compatible = product && scent && (product.hasVariants ? variant?.active && variant.scent.active && variant.scentId === scent.id : line.variantId === null);
    ...
    return { key: line.key, name: product?.name ?? line.name, scentName: scent?.name ?? line.scentName, quantity: line.quantity, image: product ? photograph(product, scent).src : line.image, price: product?.price ?? line.price, available: Math.max(0, stock - otherQuantity) };
  }), [cart, products, scents]);
  const addProduct = (product: Product, scent: Scent, quantity = 1) => {
    if (isLocked()) return false;
    const live = products.find(item => item.id === product.id); const active = scents.find(item => item.id === scent.id);
    if (!live || !active || !positiveInteger(quantity) || quantity > 99) return false;
    const variant = variantFor(live, active); const key = cartKey(live.id, variant?.id ?? null, active.id);
    ...
    if (inCart + quantity > stockFor(live, active) || nextQuantity > 99) { notify("В корзине уже всё доступное количество этой формы."); return false; }
    const line: StandardLine = { key, productId: live.id, variantId: variant?.id ?? null, scentId: active.id, quantity: nextQuantity, name: live.name, scentName: active.name, image: photograph(live, active).src, price: live.price };
    setCart([...cartRef.current.filter(item => item.key !== key), line]); setReceipt(null); setOrderError(""); notify(`${live.name} · ${active.name} — в корзине`); return true;
  };
  ...
  const value: Shop = { products, scents, loading, catalogError, loadCatalog, activeScent, setActiveScent, selectedScent: scents.find(scent => scent.id === activeScent) ?? scents[0], cart, items, count: cart.reduce((sum, line) => sum + line.quantity, 0), total: items.reduce((sum, line) => sum + (line.price ?? 0) * line.quantity, 0), hasCustom: items.some(item => item.customRecipe), locked: submitting || Boolean(pending), addProduct, addCustom, changeQuantity, remove, cartOpen, setCartOpen, detail, openProduct: (productId, scentId) => setDetail({ productId, scentId }), closeProduct: () => setDetail(null), submitOrder, retryOrder: () => { if (pendingRef.current) void sendOrder(pendingRef.current); }, pending, submitting, orderError, receipt, toast, dismissToast: () => setToast("") };
...
export function Catalog() {
  const shop = useShopping(); const [category, setCategory] = useState("all");
  const categories = [...new Set(shop.products.map(product => product.category).filter(Boolean))];
  const visible = shop.products.filter(product => category === "all" || product.category === category);
  return <section className="collection pad" id="collection" aria-labelledby="collection-title">
    <div className="section-heading"><div><p className="eyebrow">01 / КОЛЛЕКЦИЯ</p><h2 id="collection-title">У тишины<br />ваша форма.</h2></div><p className="section-description">Найдите аромат, который хочется<br />оставить в своём доме.</p></div>
    <div className="collection-toolbar"><div className="filters" role="group" aria-label="Категории свечей"><button type="button" className={category === "all" ? "active" : ""} aria-pressed={category === "all"} onClick={() => setCategory("all")}>Вся коллекция <sup>{String(shop.products.length).padStart(2, "0")}</sup></button>{categories.map(name => <button key={name} type="button" className={category === name ? "active" : ""} aria-pressed={category === name} onClick={() => setCategory(name)}>{name}</button>)}</div><span className="catalog-note">Ручная работа · свой аромат для каждой формы</span></div>
    {!!shop.scents.length && <div className="catalog-scents"><p className="eyebrow">ВЫБЕРИТЕ АРОМАТ И ЦВЕТ</p><div className="catalog-scent-options" role="group" aria-label="Аромат коллекции">{shop.scents.map(scent => <button key={scent.id} type="button" className="catalog-scent" aria-pressed={scent.id === shop.selectedScent?.id} onClick={() => shop.setActiveScent(scent.id)}><i style={{ background: scent.color }} /><span>{scent.name}</span></button>)}</div><p className="catalog-scent-description">{shop.selectedScent?.description}</p></div>}
    ...
    {!shop.loading && !shop.catalogError && !visible.length && <p className="catalog-message">Новая коллекция скоро появится здесь.</p>}
    <div className="product-grid" id="product-grid">{visible.map((product, index) => {
      const scent = shop.selectedScent; const photo = photograph(product, scent); const stock = scent ? stockFor(product, scent) : 0;
      return <article className="product-card" key={product.id} data-product-id={product.id} data-scent-id={scent?.id}>
        <button type="button" className={`product-image${photo.src.endsWith("hero.png") ? " black-image" : ""}`} aria-label={`Подробнее о свече ${product.name}`} onClick={() => shop.openProduct(product.id, scent?.id)} {...inspection}><img src={photo.src} alt={`${product.name}${scent ? ` · ${scent.name}` : ""}${photo.preview ? ". Образ аромата, фото формы уточняется" : ""}`} width="1254" height="1254" loading="lazy" /><span className="image-no">{String(index + 1).padStart(2, "0")} / ТИХО</span><span className="image-detail">Выбрать свечу <span aria-hidden="true">↗</span></span>{photo.preview && <span className="photo-preview-label">Образ аромата</span>}</button>
        <div className="product-category">{scent?.name ?? "СВЕЧА РУЧНОЙ РАБОТЫ"}</div><div className="product-title"><button type="button" onClick={() => shop.openProduct(product.id, scent?.id)}>{product.name}</button><span>{money(product.price)}</span></div><p>{scent?.notes.join(" · ") || product.notes || candleShapes[productShape(product)]}</p>
        <button className="quick-add" type="button" disabled={!stock || shop.locked} aria-label={`Добавить ${product.name}${scent ? `, ${scent.name}` : ""} в корзину`} onClick={() => scent && shop.addProduct(product, scent)}>+</button>{!stock && <span className="product-unavailable">{scent ? "Этот аромат пока недоступен" : "Ароматы скоро появятся"}</span>}
      </article>;
...
function CrossfadePhoto({ src, alt }: { src: string; alt: string }) {
  const [current, setCurrent] = useState(src); const [loaded, setLoaded] = useState("");
  useEffect(() => { if (loaded !== src || current === src) return; const timer = setTimeout(() => setCurrent(src), reducedMotion() ? 0 : 700); return () => clearTimeout(timer); }, [loaded, src, current]);
  return <><img src={current} alt={alt} width="1254" height="1254" loading="lazy" />{current !== src && <img key={src} src={src} alt="" aria-hidden="true" className={`aroma-crossfade${loaded === src ? " loaded" : ""}`} onLoad={() => setLoaded(src)} width="1254" height="1254" />}</>;
}
export function Aroma() {
  const shop = useShopping(); const [choiceKey, setChoiceKey] = useState(""); const [note, setNote] = useState(0);
  const choices = useMemo(() => shop.products.flatMap(product => (product.hasVariants ? availableVariants(product).map(variant => variant.scent) : shop.scents).map(scent => ({ key: `${product.id}:${scent.id}`, product, scent }))), [shop.products, shop.scents]);
  ...
  const notes = displayed?.scent.notes ?? [];
  const group = notes.filter((_, index) => Math.min(2, Math.floor(index * 3 / Math.max(3, notes.length))) === shown.note);
  const titles = ["ПЕРВОЕ ВПЕЧАТЛЕНИЕ", "ХАРАКТЕР КОМПОЗИЦИИ", "ПОСЛЕВКУСИЕ"];
  const descriptions = ["Прислушайтесь к первому впечатлению: какие оттенки вы замечаете сразу?", "Дайте композиции раскрыться. Именно здесь можно почувствовать её характер.", "Заметьте, какое настроение остаётся с вами после знакомства с ароматом."];
  ...
  return <section className="ritual" id="ritual" aria-labelledby="ritual-title">
    <div className="ritual-visual zoom-surface" {...inspection}><CrossfadePhoto src={choice ? photograph(choice.product, choice.scent).src : "/assets/hero.png"} alt={choice ? `${choice.product.name} · ${choice.scent.name}` : "Свет свечи ТИХО"} /><div className="ritual-overlay" /><span className="ritual-word">{choice?.scent.name.toLowerCase() ?? "почувствуйте"}.</span><span className="eyebrow visual-caption">{choice ? choice.product.name : "ТИХО"}</span><span className="ritual-photo-hint">{choice && photograph(choice.product, choice.scent).preview ? "Образ аромата · фото формы уточняется" : "Наведите, чтобы рассмотреть"}</span></div>
    <div className="ritual-content"><p className="eyebrow">02 / ИСКУССТВО АРОМАТА</p><h2 id="ritual-title">У каждого вечера —<br /><span>свой шлейф.</span></h2><label className="aroma-picker-label" htmlFor="ritual-aroma">Какой аромат раскроем?</label><div className="aroma-select-wrap"><select id="ritual-aroma" value={choice?.key ?? ""} disabled={!choices.length} onChange={event => { setChoiceKey(event.target.value); setNote(0); }}>{!choices.length && <option value="">{shop.loading ? "Загружаем ароматы…" : "Коллекция готовится"}</option>}{choices.map(item => <option key={item.key} value={item.key}>{item.product.name} — {item.scent.name}</option>)}</select><span aria-hidden="true">⌄</span></div><p className="ritual-intro">{choice?.scent.description || "Узнайте аромат чуть ближе. Выберите свечу из коллекции и неспешно пройдите её три главы."}</p><p className="note-instruction">Выберите главу, чтобы исследовать аромат</p>
      ...
      <div ref={panelRef} className={`note-panel${fading ? " is-changing" : ""}`} id="note-panel" role="tabpanel" aria-labelledby={`note-tab-${note}`} tabIndex={0}><span className="eyebrow">{titles[shown.note]}</span><h3>{group.join(" · ") || ["Знакомство", "Настроение", "Тихий след"][shown.note]}</h3><p>{descriptions[shown.note]}</p><p className="note-accuracy">Ольфакторный портрет: ноты из описания мастерской распределены по трём главам для знакомства. Это не подтверждённая пирамида раскрытия.</p></div>
      <button type="button" className="text-link" disabled={!choice} onClick={() => choice && shop.openProduct(choice.product.id, choice.scent.id)}>Выбрать эту свечу <span aria-hidden="true">↗</span></button>
    </div></section>;
}

function ProductDetails() {
  const shop = useShopping(); const product = shop.products.find(product => product.id === shop.detail?.productId);
  const [scentId, setScentId] = useState(shop.detail?.scentId); const [quantity, setQuantity] = useState(1);
  if (!product) return null;
  const profiles = product.hasVariants ? availableVariants(product).map(variant => variant.scent) : shop.scents;
  const scent = profileFor(product, shop.scents, scentId); const variant = scent ? variantFor(product, scent) : undefined;
  ...
  const available = scent ? Math.max(0, stockFor(product, scent) - inCart) : 0; const photo = photograph(product, scent);
  return <Modal className="product-dialog" label={`${product.name} — выбор свечи`} onClose={shop.closeProduct}><button type="button" className="icon-button detail-close" aria-label="Закрыть карточку свечи" onClick={shop.closeProduct}>×</button><div className={`detail-photo${photo.src.endsWith("hero.png") ? " black" : ""}`}><img src={photo.src} alt={`${product.name}${photo.preview ? ". Образ аромата, фото формы уточняется" : ""}`} /></div><div className="detail-content"><div className="detail-kicker"><span className="eyebrow">СВЕЧА РУЧНОЙ РАБОТЫ</span><span className="eyebrow">ТИХО</span></div><h2>{product.name}</h2><p>{product.notes || scent?.description}</p><label className="detail-scent-label" htmlFor="detail-scent">Аромат и цвет</label><select id="detail-scent" value={scent?.id ?? ""} disabled={!profiles.length} onChange={event => { setScentId(Number(event.target.value)); setQuantity(1); }}>{!profiles.length && <option value="">Ароматы скоро появятся</option>}{profiles.map(profile => <option key={profile.id} value={profile.id}>{profile.name} · {profile.colorName}{stockFor(product, profile) === 0 ? " · нет в наличии" : ""}</option>)}</select><dl className="detail-specs"><div><dt>Ноты</dt><dd>{scent?.notes.join(" · ") || "Уточняются"}</dd></div><div><dt>Цвет</dt><dd>{scent?.colorName ?? "Уточняется"}</dd></div><div><dt>Форма</dt><dd>{candleShapes[productShape(product)]}</dd></div><div><dt>Доступно</dt><dd>{available ? `${available} шт.` : "Нет в наличии"}</dd></div></dl><div className="detail-footer"><span>{money(product.price)}</span><div className="qty-picker" aria-label="Количество свечей"><button type="button" aria-label="Уменьшить количество" disabled={quantity <= 1} onClick={() => setQuantity(value => value - 1)}>−</button><output aria-live="polite">{quantity}</output><button type="button" aria-label="Увеличить количество" disabled={quantity >= Math.min(99, available)} onClick={() => setQuantity(value => value + 1)}>+</button></div></div><button type="button" className="button button-dark" disabled={!scent || quantity > available || shop.locked} onClick={() => { if (scent && shop.addProduct(product, scent, quantity)) shop.closeProduct(); }}>Добавить в корзину <span aria-hidden="true">+</span></button>{photo.preview && <p className="detail-notice">На изображении — образ аромата. В заказе будет форма «{candleShapes[productShape(product)]}», цвет «{scent?.colorName ?? "по выбору"}». Фотографию готовой свечи можно уточнить у мастерской.</p>}<p className="detail-notice">После оформления мастерская подтвердит заказ и согласует доставку. Онлайн-оплата на сайте не списывается.</p></div></Modal>;
}
...
export function CartOverlay() {
  ...
  return <>{shop.detail && <ProductDetails key={`${shop.detail.productId}:${shop.detail.scentId}`} />}
