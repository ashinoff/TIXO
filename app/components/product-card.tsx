...
export function ProductCard({ product, index, scent, color, favorite, onFavorite, onAdd }: {
  ...
  return <article className="product-card" data-product-id={product.id} data-scent-id={scent?.id} style={{ "--candle-color": color?.hex ?? "#f7f5ef", "--card-delay": `${Math.min(index, 5) * 60}ms` } as CSSProperties}>
    <div className="product-art">
      <CandlePreview shape={productShape(product)} color={color?.hex} label={`${product.name} — предпросмотр, ${color?.name ?? "цвет свечи"}`} />
    ...
    <div className="product-info">
      ...
      <p className="chosen-scent"><i style={{ background: color?.hex }} />{scent?.name ?? "Ароматы скоро появятся"}</p>
    ...
    {photoOpen && photo && <Modal className="form-photo-modal" label={`Фото формы ${product.name}`} onClose={() => setPhotoOpen(false)}><button className="form-photo-close" onClick={() => setPhotoOpen(false)} aria-label="Закрыть фото">×</button><img src={photo} alt={product.name} /><h3>{product.name}</h3>{product.notes && <p>{product.notes}</p>}<p>Фотография формы. Выбранный цвет — {color?.name.toLowerCase()}.</p></Modal>}
  </article>;
}
