...
import { money, type Product, type Scent, type OrderItem } from "@/lib/catalog";
...
export default function Admin() {
  ...
  const load = async () => {
    const [p, s, o, c] = await Promise.all([request<Product[]>("/api/products?admin=1"), request<Scent[]>("/api/scents"), request<Order[]>("/api/orders"), request<Record<string, { value:string; kind:string }>>("/api/content")]);
    setProducts(p); setScents(s); setOrders(o); setContent(c);
  };
  useEffect(() => { request<{ authenticated: boolean }>("/api/admin/session").then(async result => { setAuth(result.authenticated); if (result.authenticated) await load(); }).catch(cause => { setAuth(false); setError(cause instanceof Error ? cause.message : "Ошибка загрузки"); }); }, []);
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-workspace"><header className="admin-header"><div><span className="admin-kicker">Управление магазином</span><h1>{labels[tab]}</h1></div><div className="admin-actions"><Link href="/" target="_blank">Открыть сайт ↗</Link>{tab === "products" && <button onClick={() => setEditing({ ...empty })}>Добавить форму</button>}{tab === "scents" && <button onClick={() => setEditingScent({ ...emptyScent })}>Добавить аромат</button>}</div></header>
      {error && <p className="editor-error" role="alert">{error}<button type="button" className="text-action" disabled={busy} onClick={() => void perform(load)}>Обновить данные</button></p>}
      {tab === "scents" && <><p className="admin-intro">Один аромат — один цвет для всей коллекции. Создайте композиции здесь, затем добавьте их в нужные формы свечей.</p>{scents.length === 0 ? <div className="empty-admin"><h2>Начните с первого аромата</h2><p>Укажите его название, ноты и цвет вашей свечи.</p><button className="save" onClick={() => setEditingScent({ ...emptyScent })}>Создать аромат</button></div> : <div className="scent-admin-grid">{scents.map(scent => <article className="scent-admin-card" key={scent.id}><div className="scent-admin-color" style={{ background: scent.color }}><span>{scent.colorName}</span></div><div className="scent-admin-body"><span className="admin-kicker">{scent.active ? "На сайте" : "Отключён"} · {products.filter(p => p.variants.some(v => v.scentId === scent.id)).length} форм</span><h2>{scent.name}</h2><p>{scent.description}</p><div className="category-notes">{scent.notes.map((note, index) => <span key={`${note}-${index}`}>{note}</span>)}</div><footer><button onClick={() => setEditingScent(scent)}>Редактировать</button><button disabled={busy} onClick={() => removeScent(scent)}>Удалить</button></footer></div></article>)}</div>}</>}
      {tab === "products" && <><p className="admin-intro">Форма хранит название и цену. Внутри — ароматы с фотографиями и отдельными остатками.</p><div className="admin-stats"><Stat label="Форм" value={String(products.length)} note={`${products.filter(p => p.published).length} опубликовано`} /><Stat label="В наличии" value={String(products.reduce((sum, p) => sum + p.stock, 0))} note="свечей всех вариантов" /><Stat label="Ароматов" value={String(scents.filter(s => s.active).length)} note="в общем справочнике" /></div>{products.some(p => !p.hasVariants) && <p className="editor-hint">{products.filter(p => !p.hasVariants).length} форм пока продаются в исходном варианте. Откройте карточку, чтобы добавить ароматы и распределить остатки.</p>}<div className="admin-table-card"><div className="table-toolbar"><label><input aria-label="Поиск по формам и ароматам" value={query} onChange={event => setQuery(event.target.value)} placeholder="Форма или аромат" /></label><span>{filtered.length} позиций</span></div><div className="admin-table-wrap"><table><thead><tr><th>Форма</th><th>Цена</th><th>Остаток</th><th>Статус</th><th><span className="sr-only">Действия</span></th></tr></thead><tbody>{filtered.map(product => <tr key={product.id}><td><div className="product-cell"><div className="product-preview">{product.image ? <img src={product.image} alt="" /> : <span>Т</span>}</div><div><strong>{product.name}</strong><div className="table-scents">{product.variants.length ? product.variants.map(variant => <i key={variant.id} className={!variant.active || !variant.scent.active ? "muted" : ""} style={{ background: variant.scent.color }} title={`${variant.scent.name}: ${variant.stock} шт.`} />) : <small>Исходный вариант</small>}</div></div></div></td><td>{money(product.price)}</td><td>{product.stock} шт.</td><td>{product.published ? "На сайте" : "Черновик"}</td><td><button className="edit-button" aria-label={`Редактировать ${product.name}`} onClick={() => setEditing(product)}>Изменить</button></td></tr>)}</tbody></table>{!filtered.length && <p className="empty-admin">Формы не найдены</p>}</div></div></>}
