        ...
        <div className="orders-list">{orders.length === 0 ? <div className="empty-admin">Заказов пока нет</div> : orders.map(order => <article className="order-card" key={order.id}>
          ...
          <div className="order-grid">
            ...
            <div className="order-items">{order.items.map((item, index) => <div className="order-line" key={`${order.id}-${item.productId}-${item.variantId ?? index}`}>
              {item.image ? <img src={item.image} alt="" /> : (!item.customRecipe || item.customRecipe.formId !== undefined) && <div className="order-candle-preview"><CandlePreview silhouette={item.silhouette} shape={item.shape} color={item.color} twoTone={item.twoTone} accentColor={item.accentColor} label={item.name} /></div>}
              <div>
                ...
                {item.customRecipe ? <dl className="order-recipe">
                  ...
                  <div><dt>{item.accentColorName ? "Корпус" : "Цвет"}</dt><dd>{item.colorName ?? atelierColors[item.customRecipe.color]}</dd></div>{item.accentColorName && <div><dt>Декор</dt><dd>{item.accentColorName}</dd></div>}
                ...
                </dl> : item.scentName && <small><i className="order-swatch" style={{ background: item.color }} />{item.scentName} · {item.colorName}{item.accentColorName ? ` / ${item.accentColorName}` : ""}</small>}
              </div>
...
function Inventory({ products, forms, colors, scents, onEdit, onStock, onDelete, busy }: { products: Product[]; forms: CandleForm[]; colors: CandleColor[]; scents: Scent[]; onEdit: (product: Product) => void; onStock: (product: Product) => void; onDelete: (product: Product) => void; busy: boolean }) {
  ...
  const filtered = useMemo(() => products.filter(product => `${product.name} ${product.color?.name ?? ""} ${product.accentColor?.name ?? ""} ${product.scent?.name ?? ""}`.toLowerCase().includes(query.toLowerCase()) && (!formId || product.formId === Number(formId)) && (!colorId || product.colorId === Number(colorId) || product.accentColorId === Number(colorId)) && (!scentId || product.scentId === Number(scentId)) && (!stockFilter || (stockFilter === "empty" ? product.stock === 0 : stockFilter === "low" ? product.stock > 0 && product.stock <= 3 : product.stock > 0))), [products, query, formId, colorId, scentId, stockFilter]);
  const groups = useMemo(() => {
    const result = new Map<string, Product[]>();
    for (const product of filtered) {
      const key = product.formId && product.colorId && product.scentId ? `${product.formId}:${product.colorId}:${product.accentColorId ?? "none"}:${product.scentId}` : `incomplete:${product.id}`;
    ...
    <div className="admin-table-card"><div className="table-toolbar"><label><input aria-label="Поиск свечей" value={query} onChange={event => setQuery(event.target.value)} placeholder="Форма, цвет или аромат" /></label><span>{filtered.length} позиций · {filtered.reduce((sum, p) => sum + p.stock, 0)} шт.</span></div>
      ...
      <div className="inventory-table inventory-groups">{groups.map(([key, candles]) => {
        ...
        return <section className="inventory-group" key={key} aria-label={`${first.name} · ${first.color?.name ?? "Цвет не выбран"} · ${first.scent?.name ?? "Аромат не выбран"}`}>
          <header className="inventory-group-heading"><div className="product-cell"><div className="product-preview">{first.image ? <img src={first.image} alt="" /> : <CandlePreview shape={first.form ? first.form.shape : productShape(first)} silhouette={first.form?.silhouette} twoTone={first.form?.twoTone} accentColor={first.accentColor?.hex} color={first.color?.hex ?? "#e8ddca"} />}</div><div><h2>{first.name}</h2><div className="inventory-combination"><span className="inventory-color">{first.color && <i className="admin-swatch" style={{ background: first.color.hex }} />}{first.color?.name ?? "Цвет не выбран"}{first.accentColor ? ` / ${first.accentColor.name}` : ""}</span><span>{first.scent?.name ?? "Аромат не выбран"}</span></div></div></div><span className="inventory-group-total">{candles.reduce((sum, product) => sum + product.stock, 0)} шт.<small>{candles.length === 1 ? "1 позиция" : `Позиций: ${candles.length}`}</small></span></header>
