...
const emptyForm: CandleForm = { id:0, name:"", shape:"ribbed", active:true };
...
export default function Admin() {
  ...
  const saveForm = async (form: CandleForm) => {
    await request(form.id ? `/api/forms/${form.id}` : "/api/forms", { method: form.id ? "PATCH" : "POST", ...json(form) });
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-workspace"><header className="admin-header"><div><span className="admin-kicker">Управление магазином</span><h1>{labels[tab]}</h1></div><div className="admin-actions"><Link href="/" target="_blank">Открыть сайт ↗</Link>{tab === "products" && <button onClick={() => setEditing({ ...empty })}>Добавить свечу</button>}{tab === "forms" && <button onClick={() => setEditingForm({ ...emptyForm })}>Добавить форму</button>}{tab === "scents" && <button onClick={() => setEditingScent({ ...emptyScent })}>Добавить аромат</button>}{tab === "colors" && <button onClick={() => setEditingColor({ ...emptyColor })}>Добавить цвет</button>}</div></header>
      ...
      {tab === "products" && <Inventory products={products} forms={forms} colors={colors} scents={scents} onEdit={setEditing} onStock={setStockProduct} />}
      {tab === "forms" && <><p className="admin-intro">Создайте форму один раз — затем выбирайте её при добавлении и редактировании свечей.</p>{!forms.length && <p className="empty-admin">Добавьте первую форму.</p>}<div className="scent-admin-grid">{forms.map(form => <article className="scent-admin-card" key={form.id}><div className="form-card-preview"><CandlePreview shape={form.shape} color="#e8ddca" /></div><div className="scent-admin-body"><span className="admin-kicker">{form.active ? "Доступна" : "Отключена"} · {products.filter(p => p.formId === form.id).length} позиций</span><h2>{form.name}</h2><footer><button onClick={() => setEditingForm(form)}>Редактировать</button><button disabled={busy} onClick={() => removeForm(form)}>Удалить</button></footer></div></article>)}</div></>}
...
function Inventory({ products, forms, colors, scents, onEdit, onStock }: { products: Product[]; forms: CandleForm[]; colors: CandleColor[]; scents: Scent[]; onEdit: (product: Product) => void; onStock: (product: Product) => void }) {
  ...
  return <><p className="admin-intro">Одна строка — одна свеча с выбранными формой, цветом и ароматом. Заказы уменьшают доступный остаток автоматически.</p><div className="admin-stats"><Stat label="В наличии" value={String(products.reduce((sum, product) => sum + product.stock, 0))} note="свечей всего, включая черновики" /><Stat label="Заканчиваются" value={String(products.filter(product => product.stock > 0 && product.stock <= 3).length)} note="позиций с остатком 1–3 шт." /><Stat label="Нет в наличии" value={String(products.filter(product => product.stock === 0).length)} note={`из ${products.length} позиций`} /></div>
    {incomplete > 0 && <p className="inventory-notice">У {incomplete} позиций сохранён прежний общий остаток. Откройте «Изменить свечу» и выберите цвет и аромат. Количество сохранится.</p>}
    <div className="admin-table-card"><div className="table-toolbar"><label><input aria-label="Поиск свечей" value={query} onChange={event => setQuery(event.target.value)} placeholder="Форма, цвет или аромат" /></label><span>{filtered.length} позиций · {filtered.reduce((sum, p) => sum + p.stock, 0)} шт.</span></div>
      ...
      <div className="admin-table-wrap"><table className="inventory-table"><thead><tr><th>Свеча / форма</th><th>Цвет</th><th>Аромат</th><th>Цена</th><th>Остаток</th><th>На сайте</th><th><span className="sr-only">Действия</span></th></tr></thead><tbody>{filtered.map(product => <tr key={product.id}><td><div className="product-cell"><div className="product-preview">{product.image ? <img src={product.image} alt="" /> : <CandlePreview shape={productShape(product)} color={product.color?.hex ?? "#e8ddca"} />}</div><div><strong>{product.name}</strong><small>№ {product.id}</small></div></div></td><td data-label="Цвет"><span className="inventory-color">{product.color && <i className="admin-swatch" style={{ background: product.color.hex }} />}{product.color?.name ?? "Не выбран"}</span></td><td data-label="Аромат">{product.scent?.name ?? "Не выбран"}</td><td data-label="Цена">{money(product.price)}</td><td data-label="Остаток"><button className={`stock-button ${product.stock === 0 ? "empty" : product.stock <= 3 ? "low" : ""}`} onClick={() => onStock(product)} aria-label={`Изменить остаток свечи № ${product.id}`}><strong>{product.stock}</strong> шт. <span aria-hidden="true">↗</span></button></td><td data-label="На сайте"><span className={`inventory-status ${product.published ? "published" : ""}`}>{!product.published ? "Черновик" : product.form?.active === false || product.color?.active === false || product.scent?.active === false ? "Скрыта справочником" : "Опубликована"}</span></td><td><button className="edit-button" aria-label={`Редактировать свечу № ${product.id}`} onClick={() => onEdit(product)}>Изменить свечу</button></td></tr>)}</tbody></table>{!filtered.length && <p className="empty-admin">Свечи не найдены. Добавьте свечу или измените фильтры.</p>}</div>
    </div></>;
}
